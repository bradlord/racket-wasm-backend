...
// runtime. Requires apps/ide/dist/share.data to already carry the `greeter`
// package (single collection "greeter": main.rkt -> greet, util.rkt -> shout)
// -- run `cross-install --sdk <sdk> --share-data apps/ide/dist/share.data
// --dest <tmp> <greeter>` and swap the result into apps/ide/dist/ first. Not
// part of the default smoke run.
...
test('cross-installed package resolves in the booted runtime', async ({ page }) => {
  ...
  // CONSUME_TEST=1 after cross-installing greeter into apps/ide/dist/share.data.
  test.skip(!process.env.CONSUME_TEST,
    'set CONSUME_TEST=1 after cross-installing greeter into apps/ide/dist/share.data');
  await gotoIde(page);
  await bootRepl(page);
