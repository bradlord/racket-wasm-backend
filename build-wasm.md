...
- **`cross-root/`** -- `collects/` + `share/pkgs/` carrying **both sources and
  the in-place `tpb32l` `.zo`** (~47M), plus `links.rktd`/`etc`/`config`. The
...
tpb32l <dir>` + a writable `-MCR` shadow root) and repacking `share.data` with
`build/pack.rkt` -- is a separate follow-on (the cross-compile's dual host/target
module resolution has its own design). The build side (this section) is what
produces and ships the SDK.
