...
echo "[cc]  wasm_cairo.o"
emcc "${CFLAGS[@]}" "${INCS[@]}" "${CS_INCS[@]}" \
     -I "$src/build-cairo-em/install/include/cairo" \
     -o "$boot/wasm_cairo.o" -c "$src/cs/c/wasm_cairo.c"
...
link_node() {
  ...
  emcc -o "$out/scheme.html" \
       ...
       "$boot/wasm_http.o" "$boot/wasm_canvas.o" "$boot/wasm_cairo.o" \
       "${chunks[@]}" \
