#lang racket/base
;; Raw pixel-buffer-to-canvas blits. Each posts { type:"canvas", w, h,
;; pixels } from the runtime worker to the page (racket/src/cs/c/
;; wasm_canvas.c); the page's surface renders it via putImageData. The
;; REPL (browser-shell.js) appends a fresh <canvas> per blit; the
;; playground overwrites one fixed canvas. Returns 0 in the browser
;; worker, -1 under node / anywhere `self.postMessage` is unavailable.
;;
...
(define canvas-blit-rgba
  (vm-eval '(foreign-procedure "wasm_canvas_blit" (int int u8*) int)))
(define canvas-blit-argb
  (vm-eval '(foreign-procedure "wasm_canvas_blit_argb" (int int u8*) int)))
(define canvas-blit-bgra
  (vm-eval '(foreign-procedure "wasm_canvas_blit_bgra" (int int u8*) int)))
