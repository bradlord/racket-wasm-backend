...
# ---- optional deps (built earlier by build-deps.sh) ----------------
#
# build-deps.sh runs the recipe-driven library builds (libffi, libpng,
# cairo, pango, ...) and writes three artifacts into $boot:
#   wasm_deps.inc            -- foreign-symbol registrations
#   wasm_deps_uflags.txt     -- force-link flags (read below)
#   .wasm-deps-linkflags.txt -- the -L/-l flags in final link order
# build-all.sh invokes it before this script; we only consume the
# artifacts here. (rktio stays special-cased -- in-tree, built by
# build-rktio.sh per build-wasm.md §1.)

uflags_file="$boot/wasm_deps_uflags.txt"
linkflags_file="$boot/.wasm-deps-linkflags.txt"
if [ ! -f "$linkflags_file" ]; then
  echo "missing prerequisite: $linkflags_file" >&2
  echo "  -> run wasm-shell/build-deps.sh" >&2
  exit 1
fi
DEPS_LINK=()
while IFS= read -r line; do
  [ -n "$line" ] && DEPS_LINK+=("$line")
done < "$linkflags_file"
